...
fn main() {
    ...
    let min_distance = coordinates
        .iter()
        .map(|(k, _v)| k.0.abs() as usize + k.1.abs() as usize)
        .min()
        .unwrap();

    let total_min_time: usize = coordinates
        .iter()
        .map(|c| wires.iter().map(|w| w.get(c.0).unwrap()).sum::<usize>())
        .min()
        .unwrap();
